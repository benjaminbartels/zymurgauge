...
var _ temperaturecontrol.TemperatureController = (*PIDController)(nil)
...
func NewPIDTemperatureController(thermometer device.Thermometer, chiller, heater device.Actuator,
	...
	logger *logrus.Logger, options ...OptionsFunc) *PIDController {
	t := &PIDController{
...
type OptionsFunc func(*PIDController)

func SetClock(clock clock.Clock) OptionsFunc {
	return func(t *PIDController) {
		t.clock = clock
	}
}
...
func (t *PIDController) startCycle(ctx context.Context, name string, pid *pidctrl.PIDController,
...
func (t *PIDController) wait(ctx context.Context, waitTime time.Duration) bool {
...
func (t *PIDController) Run(ctx context.Context, setPoint float64) error {
	t.runMutex.Lock()
	if t.isRunning {
		defer t.runMutex.Unlock()
...
func (t *PIDController) quit(name string, actuator device.Actuator) error {
